...
class purchase_company_list(generics.GenericAPIView,mixins.ListModelMixin):
    serializer_class = Company_details_serializers
    queryset = company_details.objects.filter(purchase_company=True)
...
class supliers_contact_details_Update(generics.GenericAPIView, mixins.UpdateModelMixin,mixins.ListModelMixin, mixins.DestroyModelMixin, mixins.RetrieveModelMixin):
    serializer_class = supliers_contact_details_serializers
    queryset = supliers_contact_details.objects.all()
    lookup_field = 'id'
